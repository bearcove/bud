...
async fn main() -> Result<()> {
    ...
    match args.command {
        ...
        Some(Command::Watch) => watch_ci(),
        Some(Command::_WatchInner { pane }) => watch_ci_inner(&pane),
    }
}
...
fn watch_ci_inner(pane: &str) -> Result<()> {
    match run_watch_ci_inner(pane) {
        Ok(()) => Ok(()),
        Err(err) => {
            let _ = tmux::send_to_pane(pane, &format!("❌ CI watch failed: {err}"));
            Ok(())
        }
    }
}
...
fn run_watch_ci_inner(pane: &str) -> Result<()> {
    ...
    if watch_status.success() {
        tmux::send_to_pane(pane, "✅ CI passed.")?;
        return Ok(());
    }
